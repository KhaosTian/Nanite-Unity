...
namespace Nanite
{
    public class NaniteRendering : MonoBehaviour
    {
        ...
        private void InitBuffers()
        {
            ...
            // 输入顶点坐标缓冲区
            m_VerticesBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Vertex, m_SourceMesh.vertices.Length,
                sizeof(float) * 3);
            m_VerticesBuffer.name = nameof(m_VerticesBuffer);
            m_VerticesBuffer.SetData(m_SourceMesh.vertices);

            // 输出索引缓冲区
            m_IndicesBuffer = new ComputeBuffer(MAX_PRIMS * m_MeshletCount, sizeof(uint));
            m_IndicesBuffer.name = nameof(m_IndicesBuffer);
            m_IndicesBuffer.SetData(new int[MAX_PRIMS * m_MeshletCount]);

            // Meshlet缓冲区
            m_MeshletsBuffer = new ComputeBuffer(m_MeshletCount, sizeof(uint) * 4);
            m_MeshletsBuffer.name = nameof(m_MeshletsBuffer);
            m_MeshletsBuffer.SetData(m_Collection.meshlets);

            // Meshlet Vertices索引缓冲区
            m_MeshletVertexIndicesBuffer = new ComputeBuffer(m_Collection.vertices.Length, sizeof(uint));
            ...
            // 可见 Meshlet 索引缓冲区
            m_VisibleMeshletIndicesBuffer = new ComputeBuffer(m_MeshletCount, sizeof(uint));
        ...
        private void SetupShaders()
        {
            ...
            ProcessingCompute.SetBuffer(m_ProcessingKernelID, VisibleMeshletIndicesBufferID, m_VisibleMeshletIndicesBuffer);
            ...
            ProcessingCompute.SetBuffer(m_ProcessingKernelID, MeshletVertexIndicesBufferID, m_MeshletVertexIndicesBuffer);
            ProcessingCompute.SetBuffer(m_ProcessingKernelID, MeshletPrimitiveIndicesBufferID, m_MeshletPrimitiveIndicesBuffer);
        ...
        private void Update()
        {
            ...
            var dispatchArgs = new uint[3] { 0, 0, 0 };
            ...
            var drawArgs = new uint[5] { MAX_PRIMS, dispatchArgs[0], 0, 0, 0 };
